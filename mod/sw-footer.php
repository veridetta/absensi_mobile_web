...
<script type="text/javascript">
   var result;
    ...
    function captureimage() {
        var namaLokas = "nama tempat";
        var defLat = -6.175392;
        var defLong = 106.827153;
        var userLat = $('.latitude').html();
        var userLong = $('.longitude').html();
        var radius = 30; // dalam meter

        // konversi koordinat menjadi radian
        var defRadLat = toRadians(defLat);
        var defRadLong = toRadians(defLong);
        var userRadLat = toRadians(userLat);
        var userRadLong = toRadians(userLong);

        // hitung jarak menggunakan formula Haversine
        var dLat = userRadLat - defRadLat;
        var dLong = userRadLong - defRadLong;
        var a = Math.sin(dLat/2) * Math.sin(dLat/2) + Math.cos(defRadLat) * Math.cos(userRadLat) * Math.sin(dLong/2) * Math.sin(dLong/2);
        var c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
        var distance = 6371 * c * 1000; // jarak dalam meter

        // periksa apakah jarak kurang dari atau sama dengan radius
        if (distance <= radius) {
            console.log("Pengguna berada dalam radius " + radius + " meter dari lokasi default.");
        // play sound effect
        shutter.play();
        ...
        Webcam.snap( function(data_uri) {
            ...
            alert('Anda berada diluar jangkauan untuk melakukan absen, pastikan anda berada dalam radius '+ radius+' meter dari lokasi '+namaLokas);
            console.log("Pengguna berada di luar radius " + radius + " meter dari lokasi "+namaLokas+".");
        }
